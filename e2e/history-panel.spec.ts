...
test.describe('Story 6-2: Visualización del Historial y Panel de Actividad', () => {
  test.describe('UI: Activity Panel Toggle', () => {
    ...
    test('AC#1: Clicking activity icon opens panel with slide-in animation', async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="activity-panel"]'),
      ).not.toBeVisible();
  ...
  test.describe('UI: Panel Content', () => {
    ...
    test('AC#2: Panel shows section headers and empty live activity', async ({
      page,
    }) => {
      await loginAndNavigateToDoc(page);
      ...
      await expect(
        page.getByRole('heading', { name: 'Actividad en vivo' }),
      ).toBeVisible();
      await expect(
      ...
      await expect(
        page.locator('text=No hay otros usuarios conectados'),
      ).toBeVisible();
    });

    test('AC#3: Panel shows "No hay historial disponible" for fresh document', async ({
      page,
    }) => {
      await loginAndNavigateToDoc(page);
      ...
      await expect(
        page.locator('text=No hay historial disponible'),
  ...
  test.describe('API: Snapshot History Endpoints', () => {
    ...
    test('AC#3: GET /api/documents/:id/snapshots without auth returns 401', async () => {
      const res = await fetch(
        `${API_URL}/api/documents/${documentId}/snapshots`,
      );
