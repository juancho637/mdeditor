...
async function resetAll(): Promise<void> {
  runSQL(
    'DELETE FROM folder_permissions; DELETE FROM documents; DELETE FROM folders; DELETE FROM user_groups; DELETE FROM groups; DELETE FROM invitations;',
  );
}

async function loginAndNavigateToDoc(
...
test.describe('Story 4-3: Modo Híbrido y Cambio entre Modos', () => {
  ...
  test.describe('UI: Mode Tabs', () => {
    ...
    test('AC#1: Three mode tabs appear for editable document', async ({
      page,
    }) => {
    ...
    test('AC#2: Hybrid mode shows editor and preview side-by-side', async ({
      page,
    }) => {
      const token = adminToken;
    ...
    test('AC#2+: Typing in hybrid mode updates preview in real time', async ({
      page,
    }) => {
      const token = adminToken;
      ...
      await expect(page.locator('.prose h1')).toContainText('Live Preview', {
        timeout: 10_000,
      });
    });
    ...
    test('AC#6: Mode preference persists across page reload', async ({
      page,
    }) => {
      const token = adminToken;
      ...
      // Reload — URL persists to /dashboard/[folderId]/[docId], no re-navigation needed
      await page.reload();
      await expect(page.getByTestId('mode-tabs')).toBeVisible({
        timeout: 10_000,
      });
