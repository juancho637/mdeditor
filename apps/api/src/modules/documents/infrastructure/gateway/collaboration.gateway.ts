...
export class CollaborationGateway implements OnModuleInit {
  ...
  constructor(
    ...
    @Inject(DocumentProvidersEnum.DOCUMENT_SYNC_SERVICE)
  ...
  onModuleInit(): void {
    ...
    // Register callback so MCP write operations broadcast to WebSocket clients
    this.syncService.setOnUpdateBroadcast((documentId, update) => {
  ...
  private broadcastUpdate(documentId: string, update: Uint8Array): void {
    const clients = this.documentClients.get(documentId);
