...
import {
  ...
  DocumentInvalidatedCallback,
  DocumentUpdateBroadcastCallback,
} from '../../domain';
import {
  LoadDocumentUseCase,
  ApplyUpdateUseCase,
...
export class InMemoryDocumentSyncService
  implements DocumentSyncServiceInterface
{
  ...
  private onUpdateBroadcast?: DocumentUpdateBroadcastCallback;
  ...
  async applyExternalUpdate(
    ...
    await this.applyUpdate(documentId, update, authorId);

    if (this.onUpdateBroadcast) {
      this.onUpdateBroadcast(documentId, update);
    }
  }
  ...
  async releaseDocument(documentId: string): Promise<void> {
    ...
    if (entry.connections === 0) {
      this.logger.log(
        `Last connection closed for document ${documentId}, scheduling release`,
      );
  ...
  setOnUpdateBroadcast(callback: DocumentUpdateBroadcastCallback): void {
    this.onUpdateBroadcast = callback;
  }
  ...
  forceDocumentReload(documentId: string): void {
    ...
    if (entry) {
      ...
      this.documents.delete(documentId);
      this.logger.log(
        `Document ${documentId} forcefully invalidated for restore`,
      );
    }
  ...
  private async flushSnapshot(
    ...
    try {
      ...
      await this.persistSnapshotUseCase.run(
        documentId,
        entry.doc,
        entry.lastAuthorId,
      );
      ...
      this.logger.error(
        `Failed to persist snapshot for document ${documentId}`,
