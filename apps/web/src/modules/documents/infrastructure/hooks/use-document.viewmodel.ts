...
export function useDocumentViewModel() {
  const {
    ...
    setCurrentDocument, setFolderDocuments, setSaveStatus, setLoading, setError,
  } = useDocumentStore();
  ...
  const saveContent = useCallback(async (id: string, contentMarkdown: string) => {
    setSaveStatus('saving');
    try {
      const updated = await documentRepository.update(id, { contentMarkdown });
  ...
  const deleteDocument = useCallback(async (id: string, folderId: string) => {
    setError(null);
    try {
